 ...
 *
 * This is the code-path to use if you'd like to mount the keystone app as a sub-app in another koa application.
 *
 *   var app = koa();
 *
 *   //...do your normal koa setup stuff, add middleware and routes (but not static content or error handling middleware yet)
 *
...
function mount(mountPath, parentApp, events) {
  ...
  var less = require('koa-less');
	if (_.isArray(lessPaths)) {
		_.each(lessPaths, function(value) {
			app.use(less(this.expandPath(value), lessOptions));
		}, this);
	}

  // stylus
  var stylusPaths = this.get('stylus');
	var stylusOptions = this.get('stylus options') || {};

  if (_.isString(stylusPaths)) {
		stylusPaths = [stylusPaths];
	}

  var stylus = require('koa-stylus');
	if (_.isArray(stylusPaths)) {
		_.each(stylusPaths, function(value) {
			app.use(stylus(this.expandPath(value), stylusOptions));
		}, this);
	}
  ...
  // SASS removed in favor of Stylus!
  // ===========================
  ...
  //
	...
	// 				'\nPlease install the koa-sass from npm to use the `sass` option.' +
	// 				'\nYou can do this by running "npm install koa-sass --save".\n'
	// 			);
	// 			process.exit(1);
	// 		} else {
	// 			throw e;
  ...
  app.use(function *(){
    this.cookies.get('cookie secret');
  });
